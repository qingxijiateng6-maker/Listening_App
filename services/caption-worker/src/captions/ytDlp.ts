...
import type { CaptionProvider, CaptionProviderInput, YtDlpVideoInfo } from "./types.js";
...
type YtDlpRunner = (args: readonly string[], options?: { cwd?: string }) => Promise<YtDlpRunResult>;
...
function createDefaultRunner(binary: string): YtDlpRunner {
...
function inferSubtitleExtension(fileName: string): string | undefined {
  const parts = path.basename(fileName).split(".");
  return parts.length >= 2 ? parts[parts.length - 1]?.toLowerCase() : undefined;
}

function buildBaseArgs(cookiesPath: string | undefined): string[] {
...
function buildVideoInfoMetadata(info: YtDlpVideoInfo) {
  return {
...
export function createYtDlpCaptionProvider(
  config: WorkerConfig,
  logger: Logger,
  runner: YtDlpRunner = createDefaultRunner(config.ytDlpBinary),
): CaptionProvider {
  return {
    async fetchCaptions(input: CaptionProviderInput) {
      ...
      try {
        ...
        const info = JSON.parse(infoResult.stdout) as YtDlpVideoInfo;
        ...
        return {
          ...
          message:
            error instanceof Error ? error.message : "yt-dlp failed to fetch subtitles for this video.",
        };
      }
    },
  };
