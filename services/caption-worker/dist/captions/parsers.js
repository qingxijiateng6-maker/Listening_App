...
function parseTimedTextSeconds(raw) {
    if (!raw) {
        return null;
    }
...
function buildSegmentId(index) {
    return `seg-${String(index + 1).padStart(4, "0")}`;
}
function joinCaptionTexts(left, right) {
    if (!left) {
        return right;
    }
    if (!right) {
...
export function parseJson3Captions(rawJson) {
    const parsed = JSON.parse(stripBom(rawJson));
...
export function parseWebVttCaptions(rawVtt) {
...
export function parseTtmlCaptions(rawTtml) {
    const cues = [];
...
export function parseTimedTextXmlCaptions(rawXml) {
    const cues = [];
...
export function parseSrtCaptions(rawSrt) {
    return parseWebVttCaptions(stripBom(rawSrt).replace(/\n\d+\n/g, "\n"));
}
export function parseCaptionPayload(rawText, extensionHint) {
...
export function formatCaptionCues(cues) {
    ...
    for (const cue of normalized) {
        ...
        const shouldMerge = gapMs <= DEFAULT_MERGE_MAX_GAP_MS &&
