const SUBTITLE_FORMAT_PRIORITY = ["json3", "srv3", "srv2", "srv1", "vtt", "ttml", "srt"];
...
function matchLanguage(candidate, target) {
    if (!candidate || !target) {
...
function matchPreferredLanguage(candidate, preferredLangs) {
...
function rankFormat(formats) {
    let best = Number.MAX_SAFE_INTEGER;
    for (const format of formats) {
        const normalized = format.trim().toLowerCase();
        const index = SUBTITLE_FORMAT_PRIORITY.indexOf(normalized);
...
function isSupportedFormat(format) {
    ...
    return SUBTITLE_FORMAT_PRIORITY.includes(ext);
}
function extractTrackCandidates(trackMap, kind) {
    const candidates = [];
...
export function extractVideoLanguage(info) {
...
export function extractSubtitleTracks(info) {
    return [
...
export function buildSubtitleFormatPreference(track) {
