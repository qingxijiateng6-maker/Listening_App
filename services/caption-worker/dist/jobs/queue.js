...
const MATERIAL_PIPELINE_STEPS = ["meta", "captions", "format"];
function nowTs() {
...
function isFreshProcessingJob(job, workerId, now, lockTimeoutMs) {
    return job.status === "processing" && job.lockedBy !== workerId && !isLockStale(job.lockedAt, now, lockTimeoutMs);
}
export function computeBackoffSeconds(attempt, baseSeconds) {
    return baseSeconds * 2 ** Math.max(0, attempt - 1);
}
export function isLockStale(lockedAt, now, lockTimeoutMs) {
    if (!lockedAt) {
        return true;
    }
    return now.toMillis() - lockedAt.toMillis() > lockTimeoutMs;
}
export function buildPipelineProgressState(step, now) {
    const next = nextStep(step);
    const isCompleted = next === null;
    const materialStatus = isCompleted ? "ready" : "processing";
...
export function buildJobFailureDetails(step, attempt, error, config) {
    const nextAttempt = attempt + 1;
    const nextRunAt = Timestamp.fromMillis(Date.now() + computeBackoffSeconds(nextAttempt, config.jobBackoffBaseSeconds) * 1000);
    ...
    const detailCode = typeof error === "object" &&
        error !== null &&
        "code" in error &&
...
export class JobQueueService {
    ...
    config;
    logger;
    constructor(db, pipeline, config, logger) {
        this.db = db;
        this.pipeline = pipeline;
        this.config = config;
        this.logger = logger;
    }
    async reclaimStaleProcessingJobs(workerId) {
        const now = nowTs();
        ...
        await Promise.all(snapshot.docs.map(async (jobDoc) => {
    ...
    async lockDueJobs(limitCount, workerId) {
        const now = nowTs();
        const snapshot = await this.db
            .collection("jobs")
            .where("status", "==", "queued")
        ...
        await Promise.all(snapshot.docs.map(async (jobDoc) => {
            await this.db.runTransaction(async (tx) => {
                ...
                const duplicateSnapshot = await tx.get(this.db
                    .collection("jobs")
                    .where("type", "==", job.type)
                    ...
                    .limit(5));
                const hasOtherDone = duplicateSnapshot.docs.some((docSnap) => docSnap.id !== jobDoc.id && docSnap.data().status === "done");
                if (hasOtherDone) {
                    tx.update(jobDoc.ref, {
                        status: "done",
                        updatedAt: now,
                ...
                const hasOtherActiveProcessing = duplicateSnapshot.docs.some((docSnap) => {
                    ...
                    const duplicateJob = docSnap.data();
                    return (duplicateJob.status === "processing" &&
                        !isLockStale(duplicateJob.lockedAt, now, this.config.jobLockTimeoutMs));
                });
                if (hasOtherActiveProcessing) {
                    tx.update(jobDoc.ref, {
                        nextRunAt: Timestamp.fromMillis(now.toMillis() + this.config.jobBackoffBaseSeconds * 1000),
                        updatedAt: now,
    ...
    async lockJobById(jobId, workerId) {
        const now = nowTs();
    ...
    async dispatchJobs(limitCount, workerId) {
    ...
    async markJobQueuedForRetry(jobId, attempt, error) {
        const now = nowTs();
        const jobRef = this.db.collection("jobs").doc(jobId);
        await this.db.runTransaction(async (tx) => {
    ...
    async yieldProcessingJobIfOwned(jobId, workerId) {
        const now = nowTs();
        const jobRef = this.db.collection("jobs").doc(jobId);
        await this.db.runTransaction(async (tx) => {
            const jobSnap = await tx.get(jobRef);
    ...
    async progressMaterialPipeline(jobId, job, workerId) {
        const materialRef = this.db.collection("materials").doc(job.materialId);
        const jobRef = this.db.collection("jobs").doc(jobId);
        const now = nowTs();
    ...
    async runSingleJob(jobId, workerId) {
        const jobRef = this.db.collection("jobs").doc(jobId);
    ...
    async runJobToCompletion(jobId, workerId, maxIterations = MATERIAL_PIPELINE_STEPS.length + 2) {
        const jobRef = this.db.collection("jobs").doc(jobId);
    ...
    async dispatchAndProcessDueJobs(limitCount, workerId) {
        const dispatchResult = await this.dispatchJobs(limitCount, workerId);
        const results = await Promise.all(dispatchResult.lockedJobIds.map(async (jobId) => {
...
export function createWorkerId(prefix) {
